...
func (f fakeAdvertiser) Advertise(ctx context.Context, rec ServiceRecord) error {
	f.hub.mu.Lock()
	f.hub.records = append(f.hub.records, rec)
	f.hub.mu.Unlock()
	go func() {
		<-ctx.Done()
		f.hub.mu.Lock()
		defer f.hub.mu.Unlock()
		for i, r := range f.hub.records {
			if r == rec {
				f.hub.records = append(f.hub.records[:i], f.hub.records[i+1:]...)
				break
			}
		}
	}()
...
func (f fakeBrowser) Browse(ctx context.Context, timeout time.Duration) ([]ServiceRecord, error) {
	f.hub.mu.Lock()
	defer f.hub.mu.Unlock()
	out := make([]ServiceRecord, len(f.hub.records))
	copy(out, f.hub.records)
	return out, nil
}
